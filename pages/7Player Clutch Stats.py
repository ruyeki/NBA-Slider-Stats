...
def get_player_clutch_totals(active_player_ids, selected_year, position):
    year = selected_year
    # Add position filtering as a parameter
    season = leaguedashplayerclutch.LeagueDashPlayerClutch(season=year, player_position_abbreviation_nullable=position)
    season_clutch_regular_season = season.get_data_frames()[0]  # Get the SeasonTotalsClutchRegularSeason dataframe
    season_clutch_active_players = season_clutch_regular_season[season_clutch_regular_season['PLAYER_ID'].isin(active_player_ids)]
    filtered_season_clutch = season_clutch_active_players[columns_to_keep]
    return filtered_season_clutch
...
if __name__ == "__main__":
   ...
   # Add positions filtering
    ...
    season_totals_active_players = get_player_clutch_totals(active_player_ids, selected_year, selected_position)
...
if __name__ == "__main__":
    ...
    st.title('NBA Player Clutch Stats (Regular Season)')
   ...
   # Add positions filtering
    ...
    season_perGame_active_players = calculate_per_game_stats(get_player_clutch_totals(active_player_ids, selected_year, selected_position))

    # Sidebar options based on the selected tab
    st.sidebar.title('Choose Stats')

    st.sidebar.write("### Clutch Per Game Stats")
